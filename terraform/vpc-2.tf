...
module "vpc2_networking" {
  ...
  // Networking Settings
  vpc_name              = var.gcloud_vpc2.vpc_name
  vpc_public_subnets    = var.gcloud_vpc2.vpc_public_subnets
}

// Create the Google Cloud Firewall
module "vpc2_fw_ingress" {
  // Module Source
  ...
  // Firewall Settings
  name = var.gcloud_vpc2.ingress_rules.name
  network = module.vpc2_networking.vpc_name
  direction = var.gcloud_vpc2.ingress_rules.direction
  target_tags = var.gcloud_vpc2.ingress_rules.target_tags

  // Allow Blocks
  allow_blocks = var.gcloud_vpc2.ingress_rules.allow_blocks
}
...
module "vpc2_compute" {
  ...
  // Compute Resources
  compute_nodes         = var.gcloud_vpc2.compute
